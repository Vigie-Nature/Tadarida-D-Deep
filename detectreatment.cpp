                                ...
                                double coefspe)
{
    ...
    //
    if(_modeFreq==1) {_freqCallMin = 0.0f; _exFreqCallMin = 8.0f;}
   else
    {
        _freqCallMin=0.0f;
        _exFreqCallMin=0.8f;
...
bool DetecTreatment::openWavFile(QString& pathFile)
{
    ...
    TimeExpansion = TimeExpansion * _coefSpe;
    _detec->LogStream << "openwavfile : timexpansion = " << TimeExpansion << endl;
    _detec->LogStream << "openwavfile : coefspe = " << _coefSpe << endl;
...
bool DetecTreatment::computeFFT(QString &wavFile)
{
    ...
    MsPerX =(double)(FftHeightHalf*1000.0d * _coefSpe)/(_nbo*_soundFileInfo.samplerate*TimeExpansion); //Time: msec
    KhzPerY =(double)(_soundFileInfo.samplerate*TimeExpansion)/(double)(_fftHeight*1000.0d * _coefSpe); //Freq:khz

    _detec->LogStream << "(computefft) TimeExpansion = " << TimeExpansion << endl;
    _detec->LogStream << "(computefft) coefspe = " << _coefSpe << endl;
    _detec->LogStream << "(computefft) MsPerX = " << MsPerX << endl;
    _detec->LogStream << "(computefft) KhzPerY = " << KhzPerY << endl;
...
void DetecTreatment::detectsParameter2()
{
    ...
    _detec->LogStream << "dp2 msPerX = " << MsPerX << endl ;
    _detec->LogStream << "dp2 kHzPerY = " << KhzPerY << endl ;
    ...
    for (int icri = 0 ; icri < nbcris ; icri++) //Execute for each call
    {
            ...
            for(int j=jdeb;j<=jfin;j++)
            {
                ...
                for(int k=1;k<=3;k++)
                {
                    if((inoise<2 && _xMaxPerY[j-ymin]>0) || (inoise>1 &&  _yMaxPerX[j-xmin]>0))
                    {
                        ...
                        if(x>=0 && x<SonogramWidth && y>0 && y<FftHeightHalf && y <MAXHEIGHT)
                        {
        ...
        double e8 = 0.0d;
        int n8 = 0;
        int Y8 = (int)(_exFreqCallMin / KhzPerY);
        for(int y=0;y<Y8;y++)
        {
            qint16 *fc=SonogramArray[y];
            for(int x=xmin;x<=xmax;x++)
